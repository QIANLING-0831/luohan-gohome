...
浏览器端只将登录状态保存在 `sessionStorage`：当前标签页刷新时保持登录，关闭标签页或在新标签页重新访问时必须登录。升级时会主动清除旧版本写入 `localStorage` 的持久身份和 Token；若出现“页面身份存在但 Token 缺失”或接口返回 401，前端会立即清空假登录状态并跳转登录页。
