...
interface Props { technician: Technician; service: Service; schedule: string; onBack: () => void; onPaid: (method: string, settlement: Settlement) => void | Promise<void> }
...
export function PaymentScreen({ technician, service, schedule, onBack, onPaid }: Props) {
  ...
  const pay = () => {
    ...
    setPaying(true)
    setTimeout(async () => {
      try { await onPaid(method, { discount: coupon.discount, paidAmount, couponLabel: coupon.label }) }
      catch { /* The parent displays the domain error in a toast. */ }
