...
const demoOverview: TechnicianOverview = { technician: { id: 1, name: '陈静', title: '金牌理疗师', active: true, rating: 4.98 }, metrics: { todayOrders: 2, pendingOrders: 1, completedOrders: 862, income: 12680 }, orders: [{ id: 'LHDEMO1001', status: 'PENDING', statusIndex: 0, appointmentAt: new Date(Date.now() + 3600000).toISOString(), customer: '罗女士', phone: '138****8000', service: '肩颈深度放松', amount: 209, schedule: '今天 19:00', address: '静安嘉里中心 · 2号楼', detail: '上海市静安区南京西路1515号', intensity: '适中', note: '肩颈重点放松' }] }
...
export function TechnicianWorkbench({ user, onLogout, onChangePassword, onNotify }: { user: SessionUser; onLogout: () => void; onChangePassword: () => void; onNotify: (message: string) => void }) {
  ...
  const visibleOrders = useMemo(() => {
    if (!data) return []
    return data.orders.filter((order) => orderFilter === 'ALL' || orderFilter === 'PENDING' && order.status === 'PENDING' || orderFilter === 'ACTIVE' && order.statusIndex >= 1 && order.statusIndex <= 4 || orderFilter === 'COMPLETED' && order.status === 'COMPLETED').sort((a, b) => orderFilter === 'COMPLETED' ? new Date(b.appointmentAt).getTime() - new Date(a.appointmentAt).getTime() : orderFilter === 'ACTIVE' ? Math.abs(new Date(a.appointmentAt).getTime() - Date.now()) - Math.abs(new Date(b.appointmentAt).getTime() - Date.now()) : new Date(a.appointmentAt).getTime() - new Date(b.appointmentAt).getTime())
  }, [data, orderFilter])
  ...
  return <div className="workbench-shell"><header className="workbench-head"><div><small>罗汉到家 · 技师工作台</small><h1>{data.technician.name}，今天辛苦了</h1><p><i className={data.technician.active ? 'online-dot' : ''}/>{data.technician.active ? '当前在线接单' : '当前暂停接单'} · ★ {data.technician.rating}</p></div><div className="workspace-account-actions"><button onClick={onChangePassword}>修改密码</button><button onClick={onLogout}>退出工作台</button></div></header>
    ...
    <nav className="work-order-filters" aria-label="订单分类">{([['ALL', '全部'], ['PENDING', '待接单'], ['ACTIVE', '进行中'], ['COMPLETED', '已完成']] as const).map(([value, label]) => { const count = value === 'ALL' ? data.orders.length : value === 'PENDING' ? data.orders.filter((order) => order.status === 'PENDING').length : value === 'ACTIVE' ? data.orders.filter((order) => order.statusIndex >= 1 && order.statusIndex <= 4).length : data.orders.filter((order) => order.status === 'COMPLETED').length; return <button key={value} className={orderFilter === value ? 'active' : ''} onClick={() => setOrderFilter(value)}>{label}<small>{count}</small></button> })}</nav>
