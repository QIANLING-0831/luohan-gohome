...
export function LoginScreen({ onLogin, onNotify }: { onLogin: () => void; onNotify: (message: string) => void }) {
  ...
  const getCode = () => {
    if (!/^1\d{10}$/.test(phone)) return setError('请先输入正确的手机号')
    setError(''); setCountdown(60); onNotify('验证码已发送，演示验证码为 888888')
  }
  const submit = () => {
  ...
  return <section className="login-screen">
    ...
    <div className="login-card"><h1>欢迎回来</h1><p>登录后预约技师并查看服务进度</p>
      ...
      <div className="code-row"><div className="phone-field" style={{ margin: 0 }}><input value={code} onChange={(e) => setCode(e.target.value)} inputMode="numeric" maxLength={6}/></div><button type="button" disabled={Boolean(countdown)} onClick={getCode}>{countdown ? `${countdown}s` : '获取验证码'}</button></div>
