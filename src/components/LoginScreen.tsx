...
export function LoginScreen({ onLogin, onRegister, onNotify }: Props) {
  ...
  const selectedAccount = accounts.find((account) => account.phone === phone) ?? accounts[0]
  ...
  const getCode = () => { if (!/^1\d{10}$/.test(phone)) return setError('请先输入正确的手机号'); if (phone === '13700137000') return setError('管理员账号请使用密码登录'); setError(''); setCountdown(60); onNotify('当前是演示模式：不会发送短信，请使用 888888') }
  const submit = async () => {
