...
interface Props { user: SessionUser; profile: UserProfile | null; orders: Order[]; technicians: Technician[]; services: Service[]; onProfileChange: (value: UserProfile) => void; onLogout: () => void; onNotify: (message: string) => void; onRebook: (techId: number, serviceId: string) => void }
type Panel = 'history' | 'preferences' | 'points' | 'safety' | null

export function ProfileScreen({ user, profile, orders, technicians, services, onProfileChange, onLogout, onNotify, onRebook }: Props) {
